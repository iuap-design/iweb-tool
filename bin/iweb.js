...
var mergeWebpackConfig = function(webpackConfig, iwebConfig,rootPath, moduleName, compName){
	...
	//if (iwebConfig.exports){
	//	for (var key in iwebConfig.exports){
	//		var _path = iwebConfig.exports[key];
	//		if (_path.indexOf('./') == 0){
	//			_path = _path.substring(2,_path.length);
	//		}
	//		webpackConfig.resolve.alias[key] = tragetPath + "/" + _path;
	//	}
	//}
	if (iwebConfig.entry){
...
iweb.cli.init = function(){
	...
	process.stdin.once('data', function(name) {
		...
		process.stdin.once('data', function(desc) {
			...
			process.stdout.clearLine();
			process.stdout.cursorTo(0);
			//process.stdout.write('single-project or multi-project?(s/m):');
			//process.stdin.once('data', function(data) {
			//	process.stdout.clearLine();
			//	process.stdout.cursorTo(0);
			//	data = data.toString().trim();
			//	if (data != 's'  &&  data != 'm'){
			//		console.error('value mast be "s" or "m"');
			//		process.exit();
			//	}
				copyProject({
					name:_name,
